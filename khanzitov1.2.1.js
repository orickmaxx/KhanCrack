(function() {
    ...
    style.textContent = `
      ...
      :root {
        ...
        --khz-border: rgba(196, 181, 253, 0.2);
        --khz-primary: #a78bfa;
        --khz-primary-hover: #c4b5fd;
        --khz-text: #e0e0e0;
      ...
      .khz-button:hover { border-color: var(--khz-primary); background: rgba(167, 139, 250, 0.1); }
      .khz-button.active {
        ...
        setTimeout(() => {
          ...
          panel.innerHTML = `
            <div class="khz-header">
              ...
              <div class="khz-version">v1.3.1</div>
            </div>
          ...
          function toggleOnekoJs(isActive) {
            if (isActive) {
              loadScript('https://cdn.jsdelivr.net/gh/adryd325/oneko.js/oneko.js', 'onekoJs')
                .then(() => {
                  if (typeof oneko === "function") {
                    oneko();
                    sendToast("🐱 Gatinho ativado!");
                  } else {
                    sendToast("❌ Falha ao iniciar o Oneko.");
                    // Desativa se falhar para o estado do botão ficar correto
                    const onekoBtn = document.getElementById('khz-btn-oneko');
                    features.oneko = false;
                    if(onekoBtn) onekoBtn.classList.remove('active');
                  }
                })
                .catch(() => sendToast("❌ Não foi possível carregar o script do Oneko."));
            } else {
